...
def scan_palabras(dico):
    ...
    for key in sorted(dico.keys()):
        ...
        #print 'token'
        raw = nltk.clean_html(dico[key][1])
        print 'raw : ', raw

        # probleme d'encodage avec tokenize
        # wordpunct_tokenize au lieu de word_tokenize ne cause plus de prob d'encodage
        # mais semble moins efficace pour couper les phrases

        lista_palabras =  nltk.wordpunct_tokenize(raw)
        ...
        lista_stop_custom = ['uno', 'dos', 'tres','http']

        print lista_palabras
        for palabra in lista_palabras:
            if palabra not in spanish_stops and palabra not in lista_stop_custom \
            and len(palabra) > 2 and palabra.startswith('/')==False:
                lista_palabras_ok.append(palabra.encode('UTF8'))
...
conn = sqlite3.connect('/home/pierre/code/python/labs/Flujitos/Flujitos/bd_keywords_prensa.sqlite')          # connexion BD
...
for mot in sorted(noms_coms.keys()):
    ...
    if row:
        ...
        cur.execute("INSERT INTO flujitos_glob VALUES ( ?, ?, 1)", (mot.decode('utf8'), noms_coms.get(mot)))



for mot in sorted(noms_prop.keys()):
    # print mot
    # test s'il est déjà présent dans la BD
    arecup = (mot.decode('utf8'), )
    cur.execute('SELECT * FROM flujitos_glob WHERE mots=?', arecup)
    ...
    if row:
        # S'il est déjà présent on met à jour les occurences
        #print "Déjà présent"
        cur.execute("UPDATE flujitos_glob SET occurences = ? WHERE mots= ?", \
            (row[1] + noms_prop.get(mot), mot))
    else:
        # Sinon, on l'ajoute à la BD
        #print "Pas encore présent"
        timestamp = datetime.now()
        timestamp = timestamp.strftime('%Y-%m-%d %H:%M:%S'),
        cur.execute("INSERT INTO flujitos_glob VALUES ( ?, ?, 2)", (mot.decode('utf8'), noms_prop.get(mot)))
